...
echo "#####################################################################"
echo
bin/configtxgen -profile OrdererGenesis -outputBlock ./config/genesis.block -channelID testchannel
if [ "$?" -ne 0 ]; then
...
echo "#########################################################################"
echo
bin/configtxgen -profile ChannelBothOrgs -outputAnchorPeersUpdate ./config/OrgdlMSPanchors.tx -channelID $CHANNEL_BOTH -asOrg OrgdlMSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for OrgdlMSP..."
  exit 1
fi
